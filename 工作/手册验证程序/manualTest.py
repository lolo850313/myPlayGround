...
from excel import excelout,excelout2
...
dataDic2 = []
for i in fileList_origin:
    if i[-3:].lower() == "xml" and i[:3] == "DMC" and i.split("-")[-4] != "941A":
        dic = {}
        dic["章节"] = (i.split("-")[3])
        dic["DMC"] = i[4:34]
        ...
        dataDic2.append(dic)
title2 = ["章节","DMC","中文名称/英文名称","参引层级","被参引次数","DM到位状态","对应AIPC模块编码","对应AIPC名称","备件件号","备件件名","备件数量","单位","备件是否到位","耗材项目号","耗材名称","耗材规范号","耗材牌号","耗材库存状态","耗材是否到位","GSE件号","GSE名称","GSE是否到位","替代GSE件号","替代GSE件名","替代评估表编号"]
