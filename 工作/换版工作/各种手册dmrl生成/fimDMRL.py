...
def dmrltabelCreat(path,manualName,icCode):   
    ...
    for i in range(len(arjTask)):
        ...
        if "confltr" in arjTask[i]:
            ...
            if arjTask[i]["confltr"].strip() != "":
                dmrlInfo["Task NO."] = dmrlInfo["Task NO."] + "-" + arjTask[i]["confltr"].strip()
            print(dmrlInfo["Task NO."])
        else:
            dmrlInfo["DCV"] = "A"
        if arjTask[i]["pgblknbr"] == "2":
            dmrlInfo["IC"] = "421"
        ...
        if taskTitle == "A点火器、B离散故障－－故障隔离":
            print(taskTitle.split("－")[0])
