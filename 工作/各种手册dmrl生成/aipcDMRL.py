...
path = "/Users/hewenhao/Onedrive/工作/ATA2S1000D/aipc/"
# pathBefore = "C:\\Users\\lolo\\Desktop\\工作\\"
# path = pathBefore + "各种手册dmrl生成\\201903\\"
manualName = "AIPC-TP700016-$new$-amm.sgm"
...
def dmrltabelCreat(path, manualName):
    ...
    for i in range(len(arjTask)):
        ...
        dmrlInfo = {}
        dmrlInfo["index"] = i + 1
        dmrlInfo["Task NO."] = arjTask[i]["chapnbr"] + "-" + arjTask[i][
            "sectnbr"] + "-" + arjTask[i]["unitnbr"] + "-" + arjTask[i][
                "fignbr"]
        ...
        dmrlInfo["ICV"] = "A"
        dmrlInfo["ILC"] = "A"
        dmrlInfo["DMC"] = '=CONCATENATE(D' + str(i + 2) + ',"-",E' + str(
            i + 2) + ',"-",F' + str(i + 2) + ',"-",G' + str(
                i + 2) + ',"-",H' + str(i + 2) + ',"-",I' + str(
                    i + 2) + ',J' + str(i + 2) + ',"-",K' + str(
                        i + 2) + ',L' + str(i + 2) + ',"-",M' + str(i +
                                                                    2) + ')'
...
title = [
    "index", "Task NO.", "title", "MI", "SDC", "SYS", "SUB-SYS", "UNIT", "DC",
    "DCV", "IC", "ICV", "ILC", "DMC", "techName", "infoName", "KEY"
]
