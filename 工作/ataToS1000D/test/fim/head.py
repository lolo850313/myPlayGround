			...
			if sh.cell_value(i, 1 ) == taskNum :
				if isinstance(sh.cell_value(i, 8),str):
					result = {"modelIdentCode":str(sh.cell_value(i, 3)),
								...
								"systemCode":str(sh.cell_value(i, 5)),
...
def find_dmc(arj_task,dmrl_excel) :
	...
	taskNum = arj_task.attrib["chapnbr"] + "-" +  arj_task.attrib["sectnbr"] + "-" + arj_task.attrib["subjnbr"] +"-" + arj_task.attrib["func"] +"-" + arj_task.attrib["seq"]
	dmCode_para = findDmCode(dmrl_excel,taskNum)
	return dmCode_para["DMC"]

def headxml(arj_task,dmrl_excel,effect_dic):    
	from lxml import etree
	...
	applicRefId = effect_dic[taskEffect]
