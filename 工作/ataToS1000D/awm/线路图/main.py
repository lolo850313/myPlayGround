...
#ICN对应关系表
ICNpath = dirPath +"AWM 9月ICN-已确认1.xls"
filePath = dirPath + "wm\\"
#dmrl对应关系表
dmrl_excel = dirPath + "WM-TP700015-$new$-amm-dmrl.xlsx"

#版本，升版需要更改
issueNumber = "001"
issueDate = {"day":"20","month":"09","year":"2019"}
#dmrl中task No.所在的列
location = 1
...
for xmlPath in os.listdir(filePath):
	...
	for subject in arj_subject_list:
		...
		if subject.find("effect") is not None:
			...
			arj_subject_effect = "appRef-105+."
			if arj_subject_effect not in effect_dic:
		...
		#输出大客文档
		etree.ElementTree(root).write(dirPath + "\\awm_dmrl\\DMC-" + dmrl + '_'+issueNumber+'-00_ZH-CN.XML',xml_declaration=True,pretty_print=True,encoding="utf-8")
